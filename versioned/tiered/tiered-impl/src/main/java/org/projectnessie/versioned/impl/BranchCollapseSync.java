...
final class BranchCollapseSync<K> {
  ...
  Future<?> submitCollapse(Executor executor, K key, Runnable handler) {
    return collapses.compute(key, (k, state) -> {
  ...
  private static final class BranchCollapseState<K> {
    ...
    static <K> BranchCollapseState<K> newState(Executor executor, K key, Runnable handler,
        BiConsumer<K, BranchCollapseState<K>> finished) {
      ...
      try {
        ...
        while (true) {
          ...
          if (handler == null) {
            ...
            LOGGER.debug("Exiting work-loop for branch-collapse for ID:{} after {} runs, for {} re-schedules",
                key, runs, reSchedules);
            break;
          }
