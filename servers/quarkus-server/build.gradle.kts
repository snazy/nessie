...
listOf("quarkusApplicationModel", "quarkusApplicationDevModel").forEach { cfg ->
  tasks.named(cfg) { dependsOn("jandex") }
}
    ...
    publishing {
      publications {
        named<MavenPublication>("maven") {
          val quarkusUberJarBuild =
            tasks.named<QuarkusApplicationPackageTask>("quarkusUberJarBuild")
          artifact(quarkusUberJarBuild.flatMap { t -> t.primaryJarFile }) {
            classifier = "runner"
            builtBy(quarkusUberJarBuild)
          }
        }
      }
    }
