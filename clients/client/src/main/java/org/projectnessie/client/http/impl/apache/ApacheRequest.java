...
final class ApacheRequest extends BaseHttpRequest {
  ...
  ApacheRequest(ApacheHttpClient apacheHttpClient) {
    super(apacheHttpClient.config);
  ...
  public HttpResponse executeRequest(Method method, Object body) throws HttpClientException {
    ...
    if ((method == Method.PUT || method == Method.POST) && body != null) {
      HttpEntity entity =
          HttpEntities.create(os -> writeBody(config, os, body), ContentType.parse(contentsType));
