...
import org.projectnessie.model.{Branch, CommitMeta, ContentsKey, IcebergTable, ImmutableOperations}
...
class CommitToBranchSimulation extends Simulation {
   ...
   */
  val prometheusPushURL: Option[String] = Option(System.getProperty("sim.prometheus"))
  ...
  private def commitToBranch = {
    val chain = exec(nessie("Commit").execute { (client, session) =>
      ...
      val updatedBranch = client.getTreeApi.commitMultipleOperations(branch.getName, branch.getHash,
        ImmutableOperations.builder()
          .commitMeta(CommitMeta.fromMessage(s"test-commit $userId $commitNum"))
          .addOperations(Put.of(ContentsKey.of("name", "space", tableName), IcebergTable.of(s"path_on_disk_${tableName}_$commitNum")))
          .build())
  ...
  private def doSetUp(): SetUp = {
    val nessieProtocol: NessieProtocol = nessie()
      .prometheusPush(prometheusPushURL.map(url => PrometheusPush("commit_to_branch_simulation", url,
      ...
      .client(NessieClient.builder()
        ...
        .withTracing(prometheusPushURL.isDefined).build())
